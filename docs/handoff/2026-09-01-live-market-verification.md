...
- 앱 좌하단: `****3844` · **초록 점** · "토큰 13시간 34분 남음"
...
## 5.6 Paper 전수 검사 — 오배치 8개와 증명 장치 분리 (2026-09-01)

Paper 파일 `Athena`의 **8개 페이지를 전부** `get_tree_summary(root_node_<pageId>, depth 1)`로
열거했다(§7 함정대로 `get_children`은 100개에서 잘리므로 쓰지 않았다).

### 오배치 8개 — 번호 연속성으로 판정

세 페이지에서 보드가 **남의 페이지에 만들어져 있었다.** 추측이 아니라 **번호가 정확히
이어지는 것**이 증거다 — 원래 페이지는 중간 번호에서 끊겨 있었다.

| 보드 | 있던 곳 | 옮긴 곳 | 근거 |
|---|---|---|---|
| 백테스트 `07`·`08`·`09`·`10` | 키우미 | 백테스트 | 백테스트 페이지가 `01~06`에서 끊겨 있었다 |
| 그래프 `06`·`07` | 키우미 | 그래프 | 그래프 페이지가 `01~05`에서 끊겨 있었다 |
| 플러그인 `05`·`06` | 에이전트 | 플러그인 | 플러그인 페이지가 `01~04`에서 끊겨 있었다 |

백테스트 4장은 `KIUMI_AUDIT_STATUS.md` §3.4가 예고하고 *"그쪽 작업이 끝나면 백테스트
페이지로 옮기는 것이 맞다"*고 적어둔 것이다. 백테스트 P0~P6이 끝났으므로 이제 옮겼다.
**그래프 2장과 플러그인 2장은 어느 문서에도 기록이 없던 것**이다.

### 실앱 카드 / 증명 장치 분리

`카드` 페이지에 실제 렌더되는 카드와 커버리지 증명 장치가 섞여 있었다. 빈 채로 만들어져
있던 `증명` 페이지로 증명 장치를 옮겼다. 분류 기준은 이름 규약이다 —
**`공식 원문` · `raw` · `전체 항목`**, 접미사 `-B`(상세) · `-X`/`-Y`(공식 원문).

| 페이지 | 전 | 후 | 내용 |
|---|---:|---:|---|
| 카드 | 91 | **25** | `00 정본`, `CC-01~06`, `C01~C07`, `S00`/`S01`, `D01`, `A04`/`A06-V01` |
| 증명 | 0 | **66** | `A01-X01~X08`, `A02-Y01~Y03`, `A03-Y04~Y11`, `A04-Y01~Y04`, `A05-Y01~Y07`, `A06-Y01~Y11`, `IC-2`/`IC-3`, `DV-1~3`, `*-B` |

**실앱이 그리는 카드는 6종(CC-01~CC-06)뿐이다.** 증명 장치 66장은 렌더 대상이 아니라
"6종이 키움 299 operation을 하나도 빠뜨리지 않는다"를 눈으로 증명하는 장치다
(`verify:integrated-cards`가 같은 사실을 숫자로 낸다 — 6 카드 / 299 op / missing 0).

> 판정을 보류한 2장: `A02×A04 · 목록 펼침 검증`, `A05 · 목록 펼침 검증`. 이름에 "검증"이
> 있지만 CC 카드의 인터랙션 정본에 가까워 **카드 페이지에 그대로 뒀다.**

### 최종 배치

| 페이지 | 보드 수 | 상태 |
|---|---:|---|
| 화면 | 30 | 부팅 `02~06`이 `01` 뒤로 오도록 레이어 순서 정리 |
| 그래프 | 7 | `01~07` 연속 |
| 카드 | 25 | 정본만 |
| 에이전트 | 6 | `01~06` 연속(순서 정리) |
| 플러그인 | 6 | `01~06` 연속 |
| 키우미 | 9 | 키우미 보드만 |
| 증명 | 66 | 증명 장치만 |
| 백테스트 | 10 | `01~10` 연속 |

> 페이지 7장 → 3장 정리(`2026-09-01-paper-card-wiring.md` §7-3)는 **하지 않았다.**
> 위 전수 검사 결과 8개 페이지가 각각 뚜렷한 소유 범위를 갖고 있어, 합치면 방금 바로잡은
> 오배치가 되돌아간다. `delete_page` 도구도 없다. 목표 자체를 재검토할 것을 권한다.

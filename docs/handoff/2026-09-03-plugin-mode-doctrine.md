...
> **2026-09-03 하반 — 트랙 완료.** US-003~US-009 전 스토리가 `passes:true`이고 §7 게이트를 전부 통과했다(app 2408/2408 · backend 3222/0 · verify:plugins 154/0 · verify-graph-mode 129/0). 커밋: 2916055(main 동기화) → 51f012b(US-003) → b7fdc59(US-004) → acd6a91(US-009) → c480e5e(US-006) → 553b3bf(US-007) → c6181cc(처리됨 카드 잔류) → 아키텍트 반영·US-008. 남은 것은 PDF 검수 회신 4건(§7 표 마지막 행, 코드 앵커는 [plugin-mode-doctrine/progress.txt](./plugin-mode-doctrine/progress.txt) US-008 블록)과 main 병합 PR뿐이다. 아래 본문의 "남음/미착수" 서술은 인계 시점(상반) 기준이다.
...
**node PATH 함정(이 컴퓨터).** Bash에는 `node`·`npm`이 없다. `export PATH="/c/Users/USER/AppData/Roaming/fnm/node-versions/v22.14.0/installation:$PATH"` 후 실행한다. PowerShell은 `& $fnm env --use-on-cd --shell power-shell | Out-String | Invoke-Expression`. **backend 전수 pytest도 node가 PATH에 있어야 한다**(없으면 `test_selector_autonomous_eval.py` 실패).

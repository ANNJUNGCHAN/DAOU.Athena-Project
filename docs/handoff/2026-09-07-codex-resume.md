...
§3–6의 현재 코드 판단과 행 번호는 인수 시작 기준이다. 이후 실제 수정과 검증은 §8에 별도로 기록한다.
...
- 대응 원본은 `C:/Users/ajc22/.claude/projects/C--Projects-DAOU-Athena/0d4f88d0-d946-436b-bc39-d4e63d8412f6.jsonl`이다. 32,530,623바이트, 12,028레코드, 압축 2회를 포함한 3개 문맥이다.
...
1. **현재 재현한 HIGH 세션 오염부터 수정**한다. 실패 회귀테스트 → 최소 수정 → 관련/전체 앱 단위검사와 결정론 게이트 → 작성자와 분리된 검토 → 커밋·푸시 순서다. 이번 직접 재현 경로는 §8의 `bc4bdc2`에서 수정·검증했다. 다음에는 §8의 미검증 비동기 후보를 실제 재현으로 분류한다.
2. 외부 제목 출처 경계, `446V-1` 누락 패널, XI-0 안내·불안정 검사를 각각 실제 코드와 테스트에 연결해 닫는다. 과거 승인 거부 건을 minor로 일괄 낮추지 않는다.
...
$athenaNodeDir = 'C:/Users/ajc22/AppData/Roaming/fnm/node-versions/v22.14.0/installation'

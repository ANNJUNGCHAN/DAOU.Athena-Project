# Ralph 프롬프트 — 다른 컴퓨터에서 이어받기 (2026-09-01 작성)

아래 블록을 통째로 복사해 새 컴퓨터의 Claude Code에 붙여넣는다.
저장소를 clone한 뒤 **저장소 루트에서** 실행한다.

> ⚠ **2026-09-03 확인 — 그대로 쓰면 안 되는 부분이 있다.**
> 이 프롬프트는 "브랜치가 main 하나뿐이고 진행 중인 작업이 없는" 09-01 상태를 전제로 쓰였다.
> 지금은 진행 중인 브랜치 5개와 워크트리 3개가 있다(`docs/handoff/README.md` §1).
> 붙여넣기 전에 **§1의 브랜치 규칙과 §6-5 완료 게이트를 현재 상황에 맞게 고쳐라** —
> 안 고치면 진행 중인 남의 브랜치를 지우려 들거나, 완료 게이트가 영원히 안 닫힌다.
> §4-A 기준선 3줄도 09-01 측정치이므로 §6으로 다시 재고 갱신할 것.

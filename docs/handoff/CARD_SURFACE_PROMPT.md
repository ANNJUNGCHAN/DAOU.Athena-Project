...
현재 정확한 중단점은 적응형 가독성 **G5a**다. G1–G4는 실행·리뷰·푸시 완료(`4e4a800`·`9c11c05`·`9c07c1b`·`b77da21`·`d612cb5`). W3 Tasks 1–8·canonical 재생성도 끝난 상태라 재실행하지 말고 각 `task-{1..8}-report.md`의 honest blocker를 보존한다. coverage 3,382/3,532 · uncovered 150 · op 297/299 · validator 59건(`b15 e44`)/27장 · `STOP_NO_CHANGES`는 무변경. G4 정본 캡처는 exit 0(24장·12 probe·36 측정 전부 0)이었으나 **같은 24장 육안 전수는 FAIL**로 확정됐고, G5는 G5a(in_progress)+G5b(pending)로 대체됐다. 이 체크포인트는 G5a 코드 WIP다: 13K0-2·2R3M-1 반응형 주석, 2QFO-2 금액+단위 atomic, 2R3M-1 `3CRW-0` paired-table, `compactAtomicTokenSpans`/`legacy_pair` 검출기, 13BC-2 대조군 유지. 비-Electron 실측 scripts 265 pass, glyph/parity 54/54. **정본 Electron 캡처·육안 대조는 아직 돌리지 않았다.** 다음 단계는 정본 Electron 캡처 1회 → 시각 비교(대상 3결함 해소·13BC-2 무변경) → G5b다. zoom/전역 nowrap/overflow 숨김/보드 id CSS/글자 축소로 덮지 않고, 나머지 66장의 무라벨 접기는 범위 밖 blocker로 유지한다.
```

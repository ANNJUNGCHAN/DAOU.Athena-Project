...
현재 정확한 중단점은 적응형 가독성 **G5b**(Paper 육안 재승인)다. G1–G4는 실행·리뷰·푸시 완료(`4e4a800`·`9c11c05`·`9c07c1b`·`b77da21`·`d612cb5`). W3 Tasks 1–8·canonical 재생성도 끝난 상태라 재실행하지 말고 각 `task-{1..8}-report.md`의 honest blocker를 보존한다. coverage 3,382/3,532 · uncovered 150 · op 297/299 · validator 59건(`b15 e44`)/27장 · `STOP_NO_CHANGES`는 무변경. G4 정본 캡처는 exit 0(24장·12 probe·36 측정 전부 0)이었으나 **같은 24장 육안 전수는 FAIL**로 확정됐고, G5는 G5a(closed 자동)+G5b(pending 육안)로 대체됐다. G5a 자동 게이트는 닫힘(`00666f5`, 정본 리포트 generated_at 2026-09-03T22:49:35.105Z = 2026-09-04 07:49:35 KST). G5a 코드: 13K0-2·2R3M-1 반응형 주석, 2QFO-2 금액+단위 atomic, 2R3M-1 `3CRW-0` paired-table, `compactAtomicTokenSpans`/`legacy_pair` 검출기, 13BC-2 대조군 유지. 비-Electron 전수(scripts/app)는 이 세션에서 재실행하지 않았고 glyph/parity 54/54만 확인. **G5b는 2026-09-04 실행됐고 **승인되지 않았다**(보드×폭 11칸 중 10 PASS·1 FAIL). 결함 A(13K0-2 480/640/960, 2R3M-1 480 한글 컨트롤·제목)와 결함 C(2R3M-1 960 `3CRW-0` 라벨 붙은 paired-table)는 Paper 대조로 해소 확인됐고 13BC-2 대조군도 무변화다. 남은 블로커는 **`2QFO-2` 960px `투자자별 순매수` 표 열 정렬** 하나다 — Paper는 6열이 한 줄인데 앱은 상위 3행에서 매도 칸이 비고 값이 한 레인씩 밀리며 `44.0%`·`+3,214억원`이 라벨 없이 셋째 줄로 내려간다(`은행` 이하 행과 1920 XL은 정상). 세 하드 판정은 열-헤더 대응을 보지 않으므로 이 결함을 못 잡는다. 다음 단계는 인계 문서 §6 3번의 선택지 (a) paired-table 선언 (b) scroll-table (c) 최소 폭 완화 중 하나를 고르고 **열-헤더 대응 판정**을 게이트에 추가한 뒤 24장·12 probe를 다시 만들어 G5b를 재감사하는 것이다. 자동 0을 육안 PASS로 쓰지 말 것. `paper.jsx`를 바꾸지 말고, zoom/전역 nowrap/overflow 숨김/보드 id CSS/글자 축소로 덮지 않으며, 나머지 66장의 무라벨 접기는 범위 밖 blocker로 유지한다.
```

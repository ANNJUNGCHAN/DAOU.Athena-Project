...
- Node는 PATH에 없다: Git Bash에서 export PATH="/c/Users/ajc22/AppData/Roaming/fnm/node-versions/v22.14.0/installation:$PATH". 단위 테스트 개별 실행 가능: cd /c/Projects/DAOU.Athena/app && node --test lib/<파일>.test.js. Electron 프로브(verify:*, probe-*.js)는 직접 실행하지 마라(이미 별도 프로세스가 순차 실행 중이며 병렬 실행이 서로 망친다).

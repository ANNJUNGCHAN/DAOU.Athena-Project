...
전체 앱 검사에서 기준 커밋의 테스트 3개가 최신 구현과 불일치하는 것도 발견해
인계 브랜치에서 최소 보완했다. `grok-tool-bridge.test.js` VM에 실제 main 기준
모듈 로더를 주입했고, `session-wiring.test.js`는 확인 필요 응답을 우선하는
종료 답변 계약을 검증한다. `raw-ui-boundary.test.js`는 카드 제목 선택에 쓰는
단일 operation 메타데이터 접근만 허용하고 raw 응답 탐색 금지는 유지한다.
이 세 파일의 테스트 **27개가 통과**했다. 제품 구현을 추가로 바꾸거나 테스트를
건너뛰지 않았다. 전체 앱 검사는 최종 푸시 훅에서도 실행한다.

...
| 화면에 찍힌 결측어 | 4,712자리 · 85장 | **24자리 · 11장** |
...
## 4. 남은 결측어 24자리 — 한 갈래로 좁혀졌다

거의 전부 **저작이 자리의 뜻을 정하지 않은 값 자리**다(바인딩도 `static`도 없다,
저장소 전체에 50개 · 화면에 뜨는 것이 그중 일부). 문면이 화면 문구인지(`정규장`·
`매수체결`) 목업 숫자인지(`1,420만`·`업종 11.2배`) 코드는 알 수 없다 — 저작이 정한다.
`scripts/validate_board_slots.py`가 이미 `unmapped`로 세는 그 부채다.

나머지는 설계상 남는 자리다: 회원사 목록의 첫 항목이 그 종목 자료를 안 가진 자리
(화면이 회원사를 고른다), 주문 op가 답을 싣는 자리(하이드레이션은 주문을 호출하지
않는다).

닫은 것 중 마지막 둘: **ETF 대상지수 코드**는 그 ETF의 시간별 체결(ka40007)이 자기
대상지수 코드를 싣는다 — 연쇄 조회가 요청의 조회 대상을 물려 쓰게 해 받는다.
**실시간 대체 바인딩**은 실시간 바인딩이 하나라도 있으면 pending으로 본다.

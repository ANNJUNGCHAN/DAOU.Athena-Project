...
| 마운트 실패 보드 | 16장 | **0** |
| 가로 글자 잘림 | 37자리 · 20장 | **0** (그 20장을 다시 재서 확인) |
...
대조했다. 그 판이 **101장 중 통과 67 · 실패 34**, 지금이 **통과 68 · 실패 33**이고 실패
코드는 두 판 모두 전부 `overflow_x`다. 즉 이 빨감은 이 트랙이 만든 것이 아니고, 좁은
단계의 반응형 압축 문제다. 별 트랙(통합 카드·Paper 정합)의 몫으로 남긴다.

이 게이트가 §3의 10번을 거의 반영하지 않는 이유는 **재는 시점**이다. 넘침 처방은 값이
실린 뒤·폭이 바뀐 뒤에 재고 나서 도는데, 마운트 게이트의 정착 판정에는 그 처방이 항목으로
들어 있지 않아 처방 이전 프레임을 보는 보드가 있다(정착 판정에 넣어 봤더니 카드 1종에서
정착 한도 10초에 걸려 되돌렸다). 전수 프로브는 재기 직전에 처방을 직접 부르므로 제품이
실제로 보여주는 상태를 재고, 거기서 잘림이 0이다. 두 게이트가 어긋나 보이는 것은 이
시점 차이다.

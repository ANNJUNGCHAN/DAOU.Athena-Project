...
| 화면에 찍힌 결측어 | 4,712자리 | **1,533자리** |
| 세로 글자 잘림(스크롤로 못 닿음) | 64자리(1WOB-1) | **0** |
| 마운트 실패 보드 | 16장 | **0** |
| 가로 글자 잘림 | (안 재고 있었다) | 41자리 · 21장 — §5의 기존 빨감 |
...
## 4. 남은 결측어 — 사유별 (표본: 보드마다 최대 30노드)

`scripts/card-api-sweep/explain_missing.py` 산출.

| 사유 | 뜻 | 다음 손 |
|---|---|---|
| `row_beyond_response` | 응답 행수를 넘는 줄 | 접기가 닿지 않는 자리(줄 상자를 못 찾는 구조) 조사 |
| `array_column_no_row` | 행 좌표를 잃은 열, 유도 실패 | 보드별 저작 필요. 2QFO-2는 **열이 배열 행**인 전치 표라 규칙이 다르다 |
| `field_absent_in_response` | 업스트림이 그 필드를 안 싣는다 | 정직한 결측. 열을 지우는 것은 Paper 결정 |
| `realtime_only` | 실시간 프레임 대기 | 남은 것은 계약이 pending으로 못 잡은 자리(대체 바인딩이 있는 잎) |
| `binding_none` | 저작이 바인딩도 `static`도 안 적은 값 자리 17개 | 저작에서 마감 |
| `op_not_called_arguments:*` | 조회 대상 식별자(주문번호 등)가 없다 | 화면이 지목해야 하는 값 — 기본값 금지 |
...
즉 이 빨감은 이 트랙이 만든 것이 아니고, 좁은 단계(최소 480px 30건 · 4분할 640px 12건 ·
2분할 960px 10건)의 반응형 압축 문제다. 별 트랙(통합 카드·Paper 정합)의 몫으로 남긴다.

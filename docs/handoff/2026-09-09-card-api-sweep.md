...
| 화면에 찍힌 결측어 | 4,712자리 · 85장 | **47자리 · 18장** |
...
`scripts/card-api-sweep/explain_missing.py` 산출. 전부 「물어볼 수 없었던 자리」다.

| 사유 | 자리 | 뜻 · 다음 손 |
|---|---:|---|
| `binding_none` | 20 | 저작이 바인딩도 `static`도 안 적은 값 자리(30TY-0 8 · 2Z49-0 5 등). **저작에서 마감**한다 — 코드가 정할 수 없다 |
| `op_not_called_arguments:etfobjt_idex_cd` | 14 | ETF 대상지수 코드. 연쇄 목록(ka40004의 `trace_idex_cd`)이 빈 값이라 못 채웠다. 다른 목록 op를 찾아 연쇄를 바꾼다 |
| `op_not_called_websocket` | 6 | 실시간 op를 대체 바인딩으로 가진 잎 — pending 판정이 「전부 websocket일 때」라 안 걸린다. 대체가 실패했을 때도 pending으로 볼지 결정 필요 |
| `op_not_called_arguments:mmcm_cd` | 3 | 회원사 목록의 첫 항목(교보)이 그 종목 자료를 안 가졌다. 화면이 회원사를 고르는 자리다 |
| `op_not_called_order` | 3 | 주문 op가 답을 실는 자리. 하이드레이션은 절대 주문을 호출하지 않는다(설계) |

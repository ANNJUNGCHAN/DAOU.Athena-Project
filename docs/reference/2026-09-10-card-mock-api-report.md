...
| 6자리 가격 천단위 | `13BC-2` `267,500` · `2QRP-1` `269,500` · `2SCE-1` `268,750` |
| 순위 행 이름·코드 | `2V71-0` 목록 행은 같은 종목(이번 1위 `SK하이닉스` / `000660_AL`) |
| 순매수 `--N` | `1WOB-1` `-7만 7,222` · 1위 `-8,069` (이중 부호 없음) |
...
| 잘림·겹침 | **0** |
| 거절 TR 「미제공」 | 라이브 스윕 **0** (이 워크스페이스 로더 :8011) |
| 남은 런타임 「미제공」 | 3장 16칸 — `kt00010` `uv` · `ka10088` `ord_no` 미매핑 |

라이브 스윕은 이 워크스페이스 uvicorn `:8011`에서 돌렸다. `:8010`은 다른 세션이
잡고 있었고, 같은 `daeju`/`sangsi` 자격증명 잠금은 포트와 별개라 그 uvicorn만
내린 뒤 이 로더를 8011에 붙였다. 거절 12 TR은 호출하지 않아 그 칸은 빈 칸이다.
남은 「미제공」은 화면이 안 준 조회 인자(`uv`, `ord_no`) 때문에 물어보지 못한
자리뿐이다.
...
`ATHENA_BACKEND_BASE=http://127.0.0.1:8011 ATHENA_SWEEP_CAPTURE=* npm run verify:card-api-sweep`
(실제 `board-hydrate`, Paper 목업 주입 없음).
캡처 `app/captures/card-api-sweep/*.png` 101장, 2026-09-10 09:20–09:33 KST, 740.7s.
`CARD-API-SWEEP.json` `backend_base=http://127.0.0.1:8011`, `generated_at=2026-09-10T00:33:09Z`.

프로브는 `ATHENA_BACKEND_BASE`로 하이드레이트 대상을 바꾼다. Electron `backend-launcher`는
여전히 8010만 self-spawn하므로, 이 스윕은 self-spawn을 타지 않는 `probe-card-api-sweep.js`
경로로 돌렸다.
...
| 결함 | 고치기 전 | 이번 캡처 |
| --- | --- | --- |
| 호가 헤드라인 | `13BC-2` `269500` | `267,500` (장중 시세, 천단위) |
| 시간외 현재가 | `2QRP-1` `269500` | `269,500` |
| 보유 평단·현재가 | `2SCE-1` `268750`/`269500` | `268,750`/`267,250`, 비중 `0.2%` |
| 순위 행 이름·코드 | `2V71-0` 이름 삼성전자·코드 `252670_AL` | 목록 행은 같은 종목(1위 `SK하이닉스` / `000660_AL`). 우측 레일은 스윕 시드 이름 `삼성전자`에 1위 코드가 붙는다 |
| 순매수 `--N` | `2R3M-1` `--2860591` | 헤드라인 `267,500원` (이중 부호 없음) |
| 외국인 순매수 | `1WOB-1` `--114468977` | `-7만 7,222` · 1위 `-8,069` |
...
- `68543d0c` 버튼 전수 커밋 해시를 보고서에 보탬
- `430683c7` `:8011` 재스윕 결과를 보고서에 반영
...
# 자격증명 잠금을 이 로더가 쥔 포트로. 거절 TR missing=0. uv/ord_no 3장은 남는다
ATHENA_BACKEND_BASE=http://127.0.0.1:8011 ATHENA_SWEEP_CAPTURE=* npm run verify:card-api-sweep
```

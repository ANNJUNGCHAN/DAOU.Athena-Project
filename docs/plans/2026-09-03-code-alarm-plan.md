...
R10. **활성만 잠근다 — 일시중지는 고쳐 쓰기 허용 후 다시 검사** — 켜져 있는 알람의 코드 파일은 모델이 덮어쓰지 못하고 `POST /watch/code`가 409로 거절한다(B-24). 일시중지 알람의 파일은 고쳐 쓸 수 있다 — 잠그면 「멈추고 고치기」가 막다른 길이 된다. 고쳐 쓰면 `watch.version_hash`가 어긋나 재개가 「검사 뒤 코드가 바뀜 — 다시 검사」로 막히고, 다시 검사를 통과해야 열린다(검사는 초안·일시중지의 해시만 다시 심는다 — 켜져 있는 알람에는 경고만 돌려준다). 화면 흐름: 활성 알람에서 「고치기 — 말로」를 누르면 채팅이 먼저 멈춤을 묻고(칩 「일시중지하고 고치기」·「그대로 두기」), 사람이 멈춘 **뒤에야** 고쳐 쓰기로 넘어간다 — 409 문구는 사용자에게 노출하지 않는다(A-12).
...
- F1 ✅ `POST /api/v1/routines/watch/code` `{project_id, path, source, labels}` 신설 — `projects/store.resolve_in_project`(`store.py:140`)로 프로젝트 폴더 안 `watch/<이름>.py`에만 쓰고 `code_hash`를 돌려준다. MCP `athena_routine`에 action `propose_watch_code` 추가(또는 draft가 source를 실어 이 경로를 부른다).
  - **덮어쓰기 금지(R10)**: 활성 상태의 code-watch가 가리키는 경로만 409로 거절한다. 모델이 쓸 수 있는 것은 새 파일이거나 초안·일시중지 루틴이 가리키는 파일이다 — 일시중지는 고쳐 쓰기 통로이고, 고쳐 쓴 뒤에는 해시가 어긋나 다시 검사를 통과해야 재개된다. 409는 **사람에게 그대로 보이지 않는다** — 화면이 먼저 멈춤을 묻는다(R10 문단).
...
| B-24 | `POST /watch/code`가 **활성** code-watch가 가리키는 경로를 덮어쓰려 하면 409 + 한국어 사유(「켜져 있는 알람의 코드는 못 바꿈 — 먼저 일시중지하거나 새로 만들기」). 새 파일·초안·일시중지 참조 파일은 허용하고, 일시중지를 고쳐 쓰면 그 뒤 재개가 「검사 뒤 코드가 바뀜 — 다시 검사」로 막힌다 | `tests/api/test_routines_watch_api.py` |

...
from generate_api import SCREEN_DEFINITIONS_PATH, validate_screen_definitions
...
def mapping_row(mapping: dict[str, Any], definition: dict[str, Any]) -> str:
    ...
    cells = [
        ...
        f"`{definition['screen_id']}`",
        f"`{definition['template_case_id']}`",
...
def render_layout_section(
    ...
    category: str,
    mappings: list[dict[str, Any]],
    definitions_by_mapping: dict[str, dict[str, Any]],
) -> list[str]:
    ...
    for domain in domains:
        ...
        lines.append("")
        lines.append(
            "| Mapping ID | TR | Name | Route | Operation ID | Screen ID | Case | Req fields | Resp fields | "
            "Resp containers | Resp columns | Default control |"
        )
        lines.append("| --- | --- | --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | --- |")
        for mapping in domain_mappings:
            lines.append(mapping_row(mapping, definitions_by_mapping[mapping["mapping_id"]]))
        lines.append("")
    return lines
...
def load_screen_definitions() -> dict[str, Any]:
    if not SCREEN_DEFINITIONS_PATH.is_file():
        raise ValueError("Missing ref/kiwoom-screen-definitions.json; provisional injection maps are forbidden")
    return json.loads(SCREEN_DEFINITIONS_PATH.read_text(encoding="utf-8"))


def render(
    manifest: dict[str, Any],
    screen_definitions: dict[str, Any] | None = None,
) -> str:
    if screen_definitions is None:
        screen_definitions = load_screen_definitions()
    validate_screen_definitions(manifest, screen_definitions)
    definitions_by_mapping = {

...
def _client(handler):
    ...
    return httpx.AsyncClient(transport=transport, base_url="http://127.0.0.1:8010")
...
async def test_state_changing_actions_are_gateway_blocked(action):
...
@pytest.mark.asyncio
async def test_list_proxies_get():
...
@pytest.mark.asyncio
async def test_draft_proxies_post_and_appends_notice():
    async def handler(request):
...
@pytest.mark.asyncio
async def test_validation_error_is_translated_without_retry():
    calls = {"n": 0}
...
@pytest.mark.asyncio
async def test_backend_down_says_do_not_pretend():
    async def handler(request):

...
async def test_search_second_identical_call_skips_http(tmp_path, make_gateway):
...
async def test_describe_second_identical_call_skips_http(tmp_path, make_gateway):
    calls: list[httpx.Request] = []
...
async def test_search_different_arguments_both_hit_http(tmp_path, make_gateway):
    calls: list[httpx.Request] = []
...
async def test_search_argument_key_order_does_not_bust_cache(tmp_path, make_gateway):
...
async def test_resolve_never_cached_even_with_identical_arguments(tmp_path, make_gateway):
    calls: list[httpx.Request] = []
...
async def test_call_never_cached_even_with_identical_plan_token(tmp_path, make_gateway):
...
async def test_resolve_bypasses_even_a_cache_explicitly_shared_with_search(tmp_path, make_gateway):
    ...
    cache = SelectorCache()
    gw = make_gateway(handler, cache=cache)
...
async def test_ttl_expiry_forces_new_http_call(tmp_path, make_gateway):
    calls: list[httpx.Request] = []
...
async def test_within_ttl_still_hits_cache(tmp_path, make_gateway):
    calls: list[httpx.Request] = []
...
async def test_error_response_is_not_cached(tmp_path, make_gateway):
    calls: list[httpx.Request] = []
...
async def test_connect_error_is_not_cached(tmp_path, make_gateway):
    calls: list[httpx.Request] = []
...
async def test_success_after_earlier_error_gets_cached_normally(tmp_path, make_gateway):
...
async def test_lru_eviction_when_over_capacity(tmp_path, make_gateway):
    calls: list[httpx.Request] = []
...
async def test_lru_touch_on_get_protects_recently_used_entry(tmp_path, make_gateway):
...
async def test_catalog_version_change_invalidates_previous_entry(tmp_path, make_gateway):
    calls: list[httpx.Request] = []
...
async def test_cache_hit_recorded_in_timing_log(tmp_path, make_gateway):
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            200, json={"catalog_version": "v1", "normalized_query": "x", "results": []}
        )
...
async def test_audit_log_still_records_success_on_cache_hit(tmp_path, make_gateway):

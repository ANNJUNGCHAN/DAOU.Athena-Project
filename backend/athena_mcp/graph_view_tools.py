...
**`propose_edit`은 쓰기가 아니다.** 이 액션이 하는 일은 "이 관계를 이렇게 고칠까요?"
카드를 띄우는 것이고, **누르는 것은 사람이다.** 모델이 카드를 스스로 누를 방법은 없다.

그래프 쓰기는 세 경로이고 전부 사람의 행동에서 시작한다: 채팅→추출, 체결·잔고→결정적
투영, 그리고 **사람의 직접 편집**(2026-09-03 추가). 카드를 누르면 세 번째가 돈다 —
전용 API(`/api/v1/brain/relations/*`, 전부 `x-athena-llm-exposed:false`)가 관계를
바로 쓰고 티어는 `SourceTier.MANUAL`이다. 예전에는 답변 문장이 채팅으로 나가 추출
경로를 탔는데, 반영이 다음 수집 배치까지 밀려 사람이 같은 카드를 무한히 눌렀다
(실측). 수집(대화를 캐는 일)과 편집(주인이 화면에서 고치는 일)은 다른 일이다.
"""
...
_DESCRIPTION = (
    ...
    "제안하면 확정 카드가 뜨고, 사람이 누른 것만 그래프에 쓰인다. 그러니 '고쳤다'고 "
    "말하지 말고 '이렇게 고칠지 물었다'고 말해라."
)

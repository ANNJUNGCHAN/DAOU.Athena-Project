...
_CARD_TITLE_OVERRIDES: dict[str, str] = {
    ...
    # 시세 — quotes 도메인인데 label에 호가/주가/시세 키워드가 전혀 없어
    # title=None으로 새던 7종(2026-08-26 카드 v3 Wave 0-BE 실측). 전부 가격·
    # 체결 시계열 데이터라 "시세" 카드가 맞다.
    ...
    # 시세 — stockinfo 도메인 기본값("종목정보")으로 새던 나머지 4종. 도메인
    # 키워드로 "시세"를 추가하는 대신 개별 지정한다 — stockinfo에 "시세"
    # 키워드를 추가해 봤더니 `detail:ka10001:current_trading`(label "현재
    # 시세 및 거래량", ka10001 자기 소유 detail이라 "종목정보"가 맞다)까지
    # 걸려버리는 걸 실측으로 확인했다(2026-08-26). label에 "시세" 문자열이
    # 있는 `ka10002:market_snapshot`도 그래서 키워드가 아니라 여기 개별
    # 지정으로 넣는다.
    "base:ka10003": "시세",  # 체결정보요청
    "base:ka10055": "시세",  # 당일전일체결량요청
    "base:ka10084": "시세",  # 당일전일체결요청
    "detail:ka10002:market_snapshot": "시세",  # 종목 시세 요약
    # 거래원 — quotes/ranking 도메인인데 label에 거래원/증권사/이탈원 키워드가
    # 없어(ka10078은 "증권사별"이 붙어 있지만 도메인이 quotes라 ranking
    # 키워드 규칙이 애초에 안 걸린다) title=None으로 새던 3종.
    "base:ka10078": "거래원",  # 증권사별종목매매동향요청
    "base:ka10037": "거래원",  # 외국계창구매매상위요청
    "base:ka10062": "거래원",  # 동일순매매순위요청
    # 주문내역 — account 도메인 기본값("계좌")으로 새던 3종. kt00009의
    # 형제 detail(order_execution_status)은 이미 "주문내역"으로 정확히
    # 잡히는데 contract_amounts만 label에 매칭 키워드가 없어 새고 있었다.
    # kt50032도 마찬가지(두 detail 다 이 계열) — gold_trade_history는 원문
    # "거래 내역"(공백 있음)이 키워드 "거래내역"(공백 없음)과 정확히 안
    # 맞아 떨어져 아예 못 걸렸다.
...
_DOMAIN_LABEL_KEYWORDS: dict[str, tuple[tuple[str, str], ...]] = {
    "account": (
        ...
        ("미체결", "주문내역"),
        ("체결요청", "주문내역"),
        # 아래 3개는 "계좌"로 고친다(2026-08-26 카드 v3 Wave 0-BE 실측) — 원래
        # "주문내역"을 가리켰지만 실현손익/매매일지/거래내역 계열 6종
        # (ka10072/73/74/77, ka10170, kt00015)은 전부 계좌 단위 집계·이력이지
        # 개별 주문 목록이 아니다.
        ("실현손익", "계좌"),
        ("매매일지", "계좌"),
        ("거래내역", "계좌"),

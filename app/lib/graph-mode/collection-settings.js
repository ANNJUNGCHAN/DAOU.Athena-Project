...
// 물었을 때 답이 같은 화면 안에 있어야 한다는 뜻이다. 앱에는 같은 토글이 설정
// 오버레이(settings-cards.js `그래프` 카드, 화면 페이지 보드 22)에도 있는데,
// 둘은 **같은 저장소를 본다**(localStorage `athena.graphSettings.prefs`) — 사본이
// 아니라 같은 값의 두 입구다. 그래서 여기서 끄면 설정 화면에서도 꺼져 있다.
//

...
test('stale Claude rows are pruned and activeId follows the live credentials file', async (t) => {
  const initialState = {
    activeId: 'claude:grok.user@example.com',
    accounts: {
      'claude:user@example.com': {
        id: 'claude:user@example.com', providerId: 'claude', label: 'user@example.com',
        source: 'detected', addedAt: 'old',
      },
      'claude:grok.user@example.com': {
        id: 'claude:grok.user@example.com', providerId: 'claude', label: 'grok.user@example.com',
        source: 'detected', addedAt: 'old',
      },
    },
  };
  ...
  assert.deepEqual(claude.accounts, [
    { id: 'claude:user@example.com', label: 'user@example.com', active: true },
  ]);
  const persisted = JSON.parse(fs.readFileSync(stateFile, 'utf8'));
  assert.equal(persisted.activeId, 'claude:user@example.com');
  assert.equal(Object.prototype.hasOwnProperty.call(persisted.accounts, 'claude:grok.user@example.com'), false);
});
...
test('Grok auth.json email becomes the only grok row and can be selected', async (t) => {
  ...
  assert.deepEqual(grok.accounts, [
    { id: 'grok:grok.user@example.com', label: 'grok.user@example.com', active: true },
  ]);
  assert.equal(JSON.stringify(listed).includes('must-not-be-copied'), false);

  const activated = await accounts.setActive('grok:grok.user@example.com');
  assert.equal(activated.ok, true);
  const active = accounts.peekActiveAccount();
  assert.deepEqual(active, { accountId: 'grok:grok.user@example.com', providerId: 'grok' });
});

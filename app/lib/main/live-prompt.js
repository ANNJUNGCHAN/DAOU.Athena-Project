...
const LIVE_RULES_TEXT = [
    ...
    '주문합니다"(auto)이고 키우미 무장 스위치가 켜져 있고 그 배포가 활성이면, 신호가 날',
...
function buildBacktestModePrefix(context, today) {
  ...
  return [
    ...
    '- **"자동 매매가 되냐"고 물으면 된다고 답한다 — 다만 그것을 하는 것은 네가 아니라 사람이 건 배포다.** 배포 모드가 "한도 안에서 자동으로 주문합니다"(auto)이고 키우미 무장 스위치가 켜져 있고 그 배포가 활성이면, 신호가 날 때마다 사람 클릭 없이 한도 안에서 주문이 나간다(모의서버). 셋 중 하나라도 아니면 신호만 쌓이고 주문은 나가지 않는다 — "배포가 자동으로 주문을 냅니다"와 "제가 주문을 넣어 드립니다"는 다른 말이니 섞지 마라.',
...
function buildAgentModePrefix(context, today) {
  ...
  return [
    ...
    '② 파일을 쓰면 앱이 「함수 N개 만듦」 영수증과 「언제 확인할까요?」 질문 카드를 띄운다. 같은 질문을 산문으로 반복하지 마라. 사용자가 카드에서 고르거나 자유롭게 말한 답이 다음 메시지로 온다.',
    '③ 답이 오면 action=draft 를 부른다: symbol(6자리), condition은 {source:"code.watch", op:"==", value:true}, cooldown_s·expires_days·note(한국어 상태 한 줄), watch는 {project_id, path, version_hash: code_hash, params, poll_interval_s(고른 확인 주기), lookback_days:30}. 확인 주기를 아직 모르면 draft를 미루고 카드 답을 기다린다 — poll_interval_s:60을 지어내지 마라.',

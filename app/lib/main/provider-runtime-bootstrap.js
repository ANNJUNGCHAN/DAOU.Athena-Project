...
// 어느 환경변수를 읽을지 호출부가 정한다 — main은 이미 ATHENA_PERSISTENT_CHAT을
// claude-chat-session.js 상주 세션의 킬 스위치(기본 켜짐)로 쓰고 있다. 이 런타임은
// 같은 이름을 정반대 기본값으로 쓸 수 없어 ATHENA_PROVIDER_RUNTIME으로 분리한다.
function resolvePersistentChatEnabled(env = process.env, defaultEnabled = false, envVar = 'ATHENA_PERSISTENT_CHAT') {

...
(function () {
// 주문 티켓(주문 확인 모드)의 순수 로직 — DOM 없음, chat.js가 렌더한다.
//
// 확정 결정 2·3: 프리필은 AI(루틴)가, 실행 버튼은 사람만. 이 모듈은 새 주문
// 경로를 만들지 않는다 — 기존 백엔드 3중 게이트(주문 API 토글 + 로컬 베어러 +
// X-Athena-Confirm) 라우트로 가는 페이로드를 조립할 뿐이다. 무재시도:
// IN_DOUBT(409)는 "확인 중"으로 표시하고 절대 재전송하지 않는다(CLAUDE.md §1).
'use strict';

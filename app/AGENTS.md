...
- **Everything else is a mode of the 대화 창**, implemented as a sibling panel in `chat.html`
  ...
  - `#settings` → 설정: a sidebar nav (`#settingsNav`, 화면 · 계좌 · MCP 서버 · 모델, default
    ...
    `athena:cli-changed` broadcasts trigger a re-render. Model/effort storage is
    per-provider (2026-08-18 Codex real wiring): Claude values persist via
    `lib/main/model-prefs.js` (`athena-model.json` under userData) and feed
    `claude-runner.js buildArgs()`'s `--model`/`--effort`; Codex values are written
    directly to Codex's own `$CODEX_HOME/config.toml` (top-level `model` /
    `model_reasoning_effort` keys) via `lib/main/codex-config.js` — a global default
    that applies outside this app too, but still not consumed by this app's query path
    (only `claude -p` is wired to live queries). `main.js`'s `handleModelGet`/`handleModelSet`
    route by `provider` and merge both sources into the one `athena:model-get`/`-set`
    contract (`{claude:{model,effort}, codex:{model,effort}}`).

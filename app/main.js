...
const PERSISTENT_CHAT_DEFAULT = false;
const persistentChatEnabled = resolvePersistentChatEnabled(process.env, PERSISTENT_CHAT_DEFAULT);
...
function createProviderRuntimeControllerInstance(stateDir) {
  return createProviderRuntimeController({
    persistentEnabled: persistentChatEnabled,
...
async function resolveProviderDesiredState(options = {}) {
  ...
  if (persistentChatEnabled && (!Number.isSafeInteger(gatewayEpochRevision) || gatewayEpochRevision < 1)) {
  ...
  const mcpSnapshot = readMainMcpRuntimeSnapshot(
    config,
    persistentChatEnabled ? gatewayEpochRevision : 0,
  );
...
async function rotatePersistentProviderInner(reason, options = {}) {
  ...
  if (!persistentChatEnabled) return resolveProviderDesiredState({ ...options, activeAccount });
  if (!activeAccount) {
...
const providerConversationRotationQueue = createConversationRotationQueue({
  ...
  rotateProvider: (reason, metadata) => persistentChatEnabled
...
async function runLiveQueryInner(query, expand, origin, turnConversationId) {
  ...
  if (currentProviderSelection.disabled) {
    return {
  ...
  if (historyConversationId() === turnConversationId
    ...
    if (/session/i.test(String(result.error || ''))) {
      ...
      return runLiveQuery(query, expand, origin, turnConversationId, submit);
    }
  }
...
function abortConversationWork(reason) {
  ...
  if (providerRuntimeController && persistentChatEnabled) {
...
async function handleModelSet(e, payload = {}) {
  ...
  if (persistentChatEnabled) await rotatePersistentProvider('model_settings_changed');
...
function registerLiveBootRunners(createWindowsPromise) {
  ...
  startupReadiness.setRunner('provider-warm', async () => {
    ...
    if (!persistentChatEnabled) return { disabled: true, detail: '기존 단일 요청 대화 모드' };

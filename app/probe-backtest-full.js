...
//
// ⚠ 2026-09-08 스펙 경로 폐기 이후 이 프로브의 일부는 없는 화면을 누른다. 지도 탭·
// 시각 편집기·출처에서 지도로·"코드가 지도보다 앞섬" 갈래가 앱에서 사라졌으므로
// 그 축을 지나는 섹션은 지금 실패한다(파일 안 스펙 경로 언급 114곳). 재작성은 실백엔드
// 없이는 검증할 수 없어 같은 PR에서 하지 않았다 — 돌리기 전에 이 사실을 먼저 알 것.
// 현행 동선은 Paper 보드 19~32이고, 백엔드 없이 도는 골격 판정은 probe-backtest-mode.js다.
//

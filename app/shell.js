// 셸 부트스트랩 — Codex형 창 모델(2026-08-24 리프 1.2.1).
//
// 여기 있는 것은 **창에 속하는 것**뿐이다: 창 크롬(타이틀바·최소화/최대화/닫기),
// 창 단축키, 그리고 영역끼리 서로를 부르는 얇은 버스. 옛 판에서는 이 셋이 전부
// chat.js에 있었다 — 대화 창이 곧 앱의 창이었기 때문이다. 창이 두 영역을 담는
// 셸이 된 뒤로는 어느 한 영역의 것이 아니다.
//
// canvas.js·chat.js보다 **먼저** 로드된다(shell.html 스크립트 순서) — 두 파일이
// window.AthenaShell을 찾는다.
//
// 렌더러 격리(CLAUDE.md §6): require 없음, window.athena 다리로만 main과 통신한다.
// 렌더는 전부 textContent/DOM 노드 — innerHTML 문자열 삽입 0건(함정 ⑪).
(() => {
  'use strict';

...
**검증**: `npm run verify` 검증11(마운트·주기 탭 순서·pane 분리·지표 35행·매물대 24막대) +
수동 프로브 4종(`probe-chart-card/toolbar/indicators/authoring/drawing.js`) — 프로브 리포트·캡처는
`captures/CC-10*.json/png`. 단위 테스트는 `lib/chart-*.test.js`(순수 계산·직렬화·재투영).

...
app/
  ...
  chat.html/css/js  — 대화 창: 부팅 시퀀스, 3상태 표시, 이력, 그립 리사이즈, 창 제어 버튼
...
## 차트 카드 1판 (2026-08-18, CC-101~106 — `plan/chart-card-control-spec.md` 계약)

CompoundCard(charts) 렌더러 — 카드 12종을 늘리지 않고 `makeCard('chart', …)` 경로 하나를 추가했다.
lightweight-charts 5.2.1(ESM 전용 — CJS에선 동적 `import()`, `lib/chart-card.js` 상단 주석 실측 근거).
좌하단 로고는 Apache-2.0 라이선스가 요구하는 TradingView attribution이다 — 끄면 위반이라 유지.

**구현된 것**: 캔들+거래량 분리 pane(등락색 몸통·꼬리 동일) · 툴바(주기 탭 일·주·월·년·분·틱,
분/틱 세분, 차트모양 4종, 수정주가 토글, 확장 모드 ⛶) · 보조지표 패널(35종 전수 노출,
계산 구현 5종: 이평선·거래량MA·볼린저·RSI·MACD) · 매물대(24버킷·종가 배분·POC 강조,
우측 34% 오버레이) · 저작 상태 4종 영속(`chart.authoring.{symbol}.{period}`, localStorage 1판)
· 드로잉 2종(수평선·추세선, 확장 모드 전용, 데이터 좌표 저장).

**미구현·단순화 (정직하게 기록)**:
- 데이터는 목업 일봉 240개(`data/chart-mock-ohlcv.json`) — 실데이터 연결은 G004. 주/월/년은
  실제 집계지만 분/틱은 일봉에서 만든 **결정적 의사 재샘플**이다(카드 노트에 상시 표기).
- 수정주가 토글은 서버 보정(`upd_stkpc_tp`) 미연결 — 같은 목업을 재로드할 뿐이다(상시 표기).
- 보조지표 30종(확장 22 포함)은 목록에 노출만 — "미구현" 배지, 토글 비활성.
- 드로잉 5종(십자선·광선·사각형·타원·피보) 미구현 — 도구바에 비활성 노출.
- 저작 상태 영속은 렌더러 localStorage — 백엔드 영속·기기 간 동기화는 후속 라운드.
- "전체화면"은 OS 전체화면이 아니라 캔버스 창 안 그리드 점유다(두 창 원칙 적응 — spec §7과 의도적 차이).
- 크로스헤어 수치조회창(spec §8)·확장 차트형식 8종은 미착수.

**검증**: `npm run verify` 검증13(마운트·주기 탭 순서·pane 분리·지표 35행·매물대 24막대 —
main 병합 시 검증11에서 재번호) + 수동 프로브 4종(`probe-chart-card/toolbar/indicators/authoring/drawing.js`)
— 프로브 리포트·캡처는 `captures/CC-10*.json/png`. 단위 테스트는 `lib/chart-*.test.js`(순수 계산·직렬화·재투영).
...
### 4단계 재정의 — 전개의 종착은 채팅바 자신이다 (2026-08-18)

사용자 지시: *"4단계에서 ATHENA가 저렇게 뜨는 게 아니라, 평소 채팅바가 그대로 점점
커지다가 그 안에 ATHENA라고 적히고 다시 채팅바로 돌아온다. 부팅창의 최대 크기는
채팅창이다."* 이전 판의 확정 화면(상단 로고 영역 + 하단 입력줄 2분할)은 폐기했다.

- **확정 화면 = 평소 채팅바의 1:1 복제**: `.boot-grip`(그립 노브) · `.boot-dot`(점) ·
  `.boot-inputbar`(입력줄 64px)가 전부 `#app` 부품과 같은 값이라, 스왑 이음새가
  보이지 않는다. 값을 바꾸면 양쪽을 같이 바꿔야 한다(chat.css 주석).
- **타임라인**: 창 확정(+620ms) → 입력줄에 ATHENA 한 자씩(+660ms~, 40ms 간격) →
  300ms 유지 → 지우고 placeholder 복귀(+1160ms) → 실제 창으로 스왑(+1360ms).
  스펙의 4단계 620ms 경계는 유지하고, 그 뒤의 "이름 쓰기"는 4단계의 내부 연출이다.
- **부팅의 최대 크기는 채팅창(chatBaseH)** — 전개는 창 높이(100%)까지고 그 이상
  커지는 상태가 없다. 텍스트 등장은 `textContent` 추가라 유리 페이드 금지 규칙과
  무관하다. 크기 원칙은 2026-08-18 2차 지시로 재확인됐다: **1~4단계 전 구간에서
  부팅 표면은 채팅창과 동일한 크기다.** `probe-boot-bounds.js` 실측 — 부팅 내내
  창 bounds 1560×204(±DPI 반올림) 고정, 이후 789px 확장은 부팅이 아니라 최초 실행
  온보딩(AT-SY-002) 전환이다(`captures/boot-bounds-probe.json`).
- 검증: 검증1b(`bootBar`) — 부팅 동안 DOM을 60ms 간격 표집해 "ATHENA를 다 썼고,
  지운 뒤 placeholder로 돌아왔다"를 단언한다(`sawFullName`·`sawPhAfterName`).
  스크린샷 대신 상태를 재는 이유는 타이핑 구간(+660~+1160ms)과 `shot()` 소요의
  레이스 때문. `prefers-reduced-motion`이면 생략 자체가 스펙 준수로 기록된다.
  2026-08-18 통과(`captures/VERIFY-REPORT.json` `bootChatOnly.bootBar`).
...
| 조작 | 방법 | 구현 |
|---|---|---|
| 창 이동 | **맨 위 타이틀바(32px, 두 창 공통)를 잡고 드래그** | 네이티브 캡션(`-webkit-app-region: drag`, 2026-08-19 표준화·같은 날 개정 — 사용자 지적으로 컨트롤 스트립 캡션은 제거, 다른 앱처럼 맨 위만) — 이동은 OS가 수행, 가장자리 끌기 스냅 포함. 구판 커서 폴링(`athena:window-drag`)은 폐기 |
| 확대/축소 | `Ctrl+=` / `Ctrl+-` / `Ctrl+0` / `Ctrl+휠` | 두 창 `zoomFactor` 동기(0.5~2.0), 높이 측정·점 좌표·클립 좌표를 배율 보정 |
| 창 내리기 | `Ctrl+M` 또는 작업 표시줄, **우상단 − 버튼** | 두 창을 한 몸으로 최소화, 한쪽 복원이 짝을 끌어올린다 |
| 다른 앱 위로 올리기 | 그냥 클릭 | `alwaysOnTop` 제거 — 스파이크 시절 값이었고 "창을 내릴 수 없다"의 원인이었다. 두 창 짝(캔버스 위 대화 창)은 focus/restore의 `moveTop()`으로만 유지 |

짝 의미론(2026-08-19 정정 5): **드래그하면 창 짝이 한 몸으로 움직인다** — OS는
잡힌 창 하나만 옮기므로 놓는 순간 main이 상대 창을 같은 델타로 정착시킨다
(`handleForeignArrange` 순수 이동 분기). 프로그램적 이동(verify의 noteAppBounds
동반 setBounds)은 여전히 독립적이다 — 검증3(독립성)과 검증11(짝 팔로우)이 각각
계약을 단언한다. 실물 마우스 캡션 드래그 자체는 자동화로 못 돌린다(검증9와 같은
제약) — 검증11은 캡션/구멍 CSS 계약 + 외부 순수 이동 재현으로 대신한다.

### 창 제어 버튼 — 우상단 − · □ · × (AT-CH-001, 2026-08-18)

사용자 지시("대화창에 화면 최소화·최대화 버튼, 그리고 닫기 버튼 — 백그라운드는
살아있음")로 보이는 크롬을 추가했다. 단축키만 있던 창 조작에 마우스 진입로가 생겼다.

- **위치·소속**: 창 우상단 고정 오버레이(`#winControls`). 특정 모드의 부품이 아니라
  **창의 크롬**이라 `#app`/`#onboard`/`#settings` 밖에 있고 모드 전환과 무관하게
  유지된다. 부팅 연출 중에는 숨겼다가 `finishBoot`에서 드러난다. 무채색만 쓴다
  (액센트 색 없음 — 닫기에도 빨강을 칠하지 않는다), 아이콘은 CSS 도형이다.
  설정 모드 헤더는 같은 코너의 안내 문구가 크롬 밑에 깔리지 않게 우측 94px를
  비운다(리뷰 지적 — `17-settings-mode.png` 겹침 실측, `.settings-head` 주석).
- **최소화(−)**: 기존 `athena:minimize-windows`(Ctrl+M과 동일) — 두 창 한 몸.
- **최대화(□)**: 이 앱에서 창의 최대는 OS 전체화면이 아니라 **설계 최대 높이
  (chatMaxH, E3)다** — 그립 드래그와 같은 높이 배선으로 chatBaseH↔chatMaxH를
  토글한다(해석 — OS `maximize()`는 두 창 짝 기하·클립 좌표 계약을 깨서 기각).
  이미 최대면 아이콘이 "복귀"(겹친 두 면)로 바뀌고, 복귀는 **라벨 그대로 기본
  높이로** 내려간다 — 자동 성장은 `manualOverride` 해제로 다음 내용 변화부터
  재개된다(리뷰 지적: 복귀 직후 `scheduleHeightSync()`를 부르면 자연 높이로 즉시
  재확장돼 이중 리사이즈만 남았다 — 제거). 온보딩·설정 모드가 높이를 소유하는
  동안은 버튼이 **비활성(디밍)** 이다 — "복귀" 아이콘을 걸어두고 클릭을 조용히
  무시하는 것은 정보 정직성 위반이라(같은 리뷰) 상태를 드러낸다.
- **닫기(×)**: 종료가 아니다 — 두 창을 숨기고 프로세스는 백그라운드에 산다
  (`athena:close-windows`). 복귀는 **트레이**(브랜드 점 아이콘, 이미지 에셋 없이
  16×16 BGRA 비트맵을 코드로 그린다): 클릭 = 열기, 메뉴 = 열기/종료. 캔버스가
  세션에서 열려 있었다면 짝으로 같이 돌아온다. `Alt+F4` 등 OS close 이벤트는
  가로채지 않는다 — 그쪽은 여전히 진짜 종료다(`closed` → `app.quit()`).
- 검증: 검증9d — 닫기 후 두 창 비표시 + 트레이 복귀 경로(`restoreFromBackground`
  동일 함수 참조 직접 호출)로 복원을 단언. 2026-08-18 통과
  (`captures/VERIFY-REPORT.json` `closeToBackground`). **정직 표기**: 실제 트레이
  아이콘 클릭 자체는 자동화로 만들 수 없어 미실측이다.
...
4. **(2026-08-18 사용자 보고) 빈 유리 표면을 클릭만 해도 창이 자랐다.** 빈 배경
   ...
   이동만 적용 — 반올림이 누적될 자리를 없앴다. **후기(2026-08-19)**: 커서 폴링
   경로 자체가 네이티브 캡션으로 대체되며 소멸했다(정정 5) — 검증11이 "옛 채널
   거부 + 크기 불변"으로 회귀 가드를 계승한다.
...
스트립·헤더·캔버스 스트립의 grab로 이동했다 — 본문 grab은 제거(본문은 선택).
...
## 능동 에이전트 — 루틴·알림·능동 턴·주문 티켓 (2026-08-19, P2~P4)

설계 정본은 `plan/능동-에이전트-실행계획-2026-08-19.md`(ralplan 합의)다.

- **수신**: `lib/main/routine-feed.js`가 백엔드 `/api/v1/ws/routines`를 구독한다
  (auth envelope 경로 — 브라우저 호환 WS는 헤더를 못 싣는다). 지수 백오프 재연결.
  fixture 모드에서는 돌지 않는다(검증 결정론 보호 — backdrop 샘플링과 같은 문법).
- **표시**: 발화·복원실패만 OS 토스트(조용한 감시자 — 만료는 이력만). 본문은
  `lib/routine-turn.js`의 결정론 템플릿(LLM 0)이라 지어낼 수 없다. 대화 창에는
  **능동 턴**(.turn-agent)이 쌓인다 — 발화 시각 배지·감시 방식 표기·"묻지 않은
  턴" 라벨이 필수 계약(시점 정직성), 등장은 backdrop-filter 굴절 변조
  (reduced-motion 즉시 표시). 렌더는 전부 textContent — innerHTML 0건 유지.
- **승인**: 모델은 `athena_routine`(draft/list)으로 제안만 한다. 질의 완료 후
  draft 목록을 재조회해 승인 카드를 띄우고, [승인]은 렌더러가 백엔드 confirm을
  직접 부른다(LLM 재스폰 없음). 방식 행("실시간 (WS) — 틱 즉시" / "주기 확인 —
  최대 폴링 주기만큼 지연")은 필수 렌더다.
- **주문 확인 모드(#order)** — 유일하게 미착수였던 모드의 최초 구현. 온보딩·설정과
  같은 형제 패널 문법(Esc 4분기 첫 순위). 프리필은 값·사유까지(방향·수량은 사람),
  집행은 기존 3중 게이트 백엔드 라우트 그대로·무재시도, IN_DOUBT(409)는 "확인 중"
  종결 상태 — 재전송 불가(`lib/order-ticket.js` 상태기계).
- **검증**: `npm run verify` 검증17이 합성 발화 → 능동 턴 계약 → 티켓 모드 전이 →
  게이트 잠금 → Esc 복귀를 쿼터 0으로 상시 검증한다.

정직 고지: 주기(periodic) 루틴의 감지는 폴링 주기만큼 늦을 수 있고, 앱·백엔드가

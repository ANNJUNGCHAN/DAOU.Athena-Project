...
async function realClick(shellWin, x, y) {
  ...
  shellWin.webContents.sendInputEvent({ type: 'mouseMove', x, y });
  await wait(80);
  shellWin.webContents.sendInputEvent({ type: 'mouseDown', x, y, button: 'left', clickCount: 1 });
  await wait(50);
  shellWin.webContents.sendInputEvent({ type: 'mouseUp', x, y, button: 'left', clickCount: 1 });
  await wait(300);
}
...
async function main() {
  ...
  mark('collapsed');
  // ---- ① 축소 상태 — 도구바 비노출 ----
  report.collapsed = await shellWin.webContents.executeJavaScript(`(() => {
  ...
  await wait(500);
  report.expanded = await shellWin.webContents.executeJavaScript(`(() => {
    const bar = document.querySelector('.card.chart .chart-drawbar');
  ...
  await wait(120);
  await realClick(shellWin, cx, cy);
  report.hline = await shellWin.webContents.executeJavaScript(`(() => {
  ...
  await shellWin.webContents.executeJavaScript('window.__drawDebug = []');
  mark('trend');
  // ---- ④ 추세선 — 도구 선택 + 2 실클릭 ----
  await shellWin.webContents.executeJavaScript(`(() => {
    const bar = document.querySelector('.card.chart .chart-drawbar');
  ...
  await wait(120);
  await realClick(shellWin, Math.round(rect.x + rect.w * 0.25), Math.round(rect.y + rect.h * 0.6));
  await realClick(shellWin, Math.round(rect.x + rect.w * 0.7), Math.round(rect.y + rect.h * 0.35));
  report.trend = await shellWin.webContents.executeJavaScript(`(() => {
    const stored = JSON.parse(localStorage.getItem('chart.authoring.005930.D') || 'null');
  ...
  await wait(400);
  report.afterExit = await shellWin.webContents.executeJavaScript(`(() => {
    const bar = document.querySelector('.card.chart .chart-drawbar');
    const stored = JSON.parse(localStorage.getItem('chart.authoring.005930.D') || 'null');
    return {
      barVisible: getComputedStyle(bar).display !== 'none',

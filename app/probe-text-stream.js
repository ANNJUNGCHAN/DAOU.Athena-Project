// S2 텍스트 스트리밍 결선 확인 프로브 — chat.js가 실제로 athena:live-text-delta를
// 턴 종료 전에 여러 번 받는지, 첫 조각이 오는 시각이 최종 invoke() resolve보다
// 확연히 이른지를 실측한다. 백엔드(127.0.0.1:8010)가 떠 있어야 한다.
...
async function main() {
  ...
  await shellWin.webContents.executeJavaScript('window.__unsubDelta && window.__unsubDelta(); undefined');
  ...
  app.exit(summary.deltaCount > 0 && summary.firstDeltaMsBeforeResolve > 0 ? 0 : 1);
}

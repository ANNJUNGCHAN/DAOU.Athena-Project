...
app.whenReady().then(async () => {
  ...
  try {
    ...
    await run(`document.body.style.background = '#f5f6f9'; document.body.innerHTML = '<main style="padding:48px;font-family:Segoe UI,sans-serif;color:#676776"><h1>알람 표시 검증</h1><p>실제 알람이 아닌 화면 검증용 데이터입니다.</p><input id="draft" aria-label="작성 중인 메시지" value="작성 중인 메시지"></main>'; document.getElementById('draft').focus();`);
    await run(fs.readFileSync(path.join(__dirname, 'lib/routine-alert-popup.js'), 'utf8'));
    await run(`window.opened = []; window.agents = []; window.mainCard = {operation_ref:'base:ka10081',args:{stk_cd:'005930',base_dt:'$today',upd_stkpc_tp:'1'},title:'주식일봉차트'};
      window.popup = window.AthenaLib.RoutineAlertPopup.createRoutineAlertPopup({ document, onOpenCard: id => opened.push(id), onOpenAgent: id => agents.push(id) });
      popup.show({ id: 'fixture-volume', title: '삼성전자 전일 대비 거래량 2배 이상 급등 시 알림', sub: '화면 검증용 예시 · 005930 · 관측 2.1', firedAt: Date.now(), mainCard });`);
    ...
    await run(`popup.show({ id:'fixture-next', title:'두 번째 알람 — 긴 제목 '.repeat(12), sub:'검증용 데이터', firedAt:Date.now(), mainCard }); popup.dismiss();`);
    ...
    await run(`popup.show({id:'legacy',title:'메인 카드 미설정 알람',firedAt:Date.now()})`);
    assert.equal(await run(`document.querySelector('.routine-alert-popup__card').disabled`), true);
    await run(`document.querySelector('.routine-alert-popup__agent').click()`);
    assert.deepEqual(await run('agents'), ['legacy'], 'Agent remains available for old alarms');
    assert.deepEqual(await run('opened'), ['fixture-next'], 'Legacy alarms must not open an invented card');

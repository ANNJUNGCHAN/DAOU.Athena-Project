...
- `probe-orb-mini-cards`는 **단독 실행에서 22/22였는데 배치에서 실패**했다("25초 안에 카드 7장이
  안 나왔다"). 배치 실행 자체가 불안정하다는 직접 증거다(실행기가 매 프로브 앞에서
  `taskkill electron`을 때리는데, 남은 프로세스 정리 타이밍과 겹친다).

**아직 확정 못 한 것: `glad 02b`.** 측정값 `5.5 × 13.5 / radius 999px`은 아치(`border-radius:
999px 999px 0 0`, 배경 투명)가 아니라 **기본 눈 모양**이다. `orb.css`의 done/glad 공용 아치
규칙과 glad 재정의(27.4%/16.4%)는 육안 확인 결과 그대로 살아 있고, `HEAD~1` 대비 표정 규칙
diff는 `drift` 한 줄 삭제뿐이다. 그래도 **CSS를 건드린 커밋이라 무죄를 단정할 수 없다.**

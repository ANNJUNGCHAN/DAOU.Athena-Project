...
- **창은 둘뿐이다. 예외 없다.** 정의는 [`GLOSSARY.md`](GLOSSARY.md) §1.
  **둘이라는 수는 불변이고, 둘의 정체는 Codex형 개정 2026-08-24로 바뀌었다.**
  - **목표 구조(Codex형)** — **셸 창** + **알림 오브 창**. 둘 다 `frame: false`.
    - **셸 창**은 3영역 한 창이다: 좌 **이력 사이드바 268px**(접힘 가능) · 중앙 **캔버스 flex**
      (유일한 출력 지점) · 우 **채팅 400px**(유일한 입력 지점, 어떤 상태에서도 접히지 않는다).
      **중앙에 뜨는 것은 "카드"다**(12종 고정). 중앙은 캔버스이지 대화 트랜스크립트가 아니다.
    - **알림 오브 창**은 76px 원형이다. `alwaysOnTop` · 자유 드래그 · 클릭으로 펼침.
      **실행 버튼 없음 · 미니 입력창 없음** — 주문은 사람이 낸다(확정 결정 3).
      펼침은 세 조각까지: 대화체 문장 1 + 대표 카드 1 + 더보기.
  - **그 외의 모든 화면은 셸 창의 모드다** — 온보딩 · 인증 · 설정 · 주문 확인 · 에이전트.
    새 창을 만들지 않는다. **설정은 셸 창 전체가 통째로 스왑**되고, 복귀는 좌상단
    "← 앱으로 돌아가기" 또는 `Esc` **둘 다 같은 동작**이다.
  - 즉시 탈락 조건: **창 3개 이상**, **접을 수 없는 사이드바**·탭바·툴바, 커맨드바 외 경로로**만**
    접근 가능한 설정.
    - Codex형 개정 2026-08-24: 좌측 **이력 사이드바**는 **접힌다**는 조건에서 허용한다.
      옛 조항("상시 사이드바 = 즉시 탈락")은 폐기했다. 근거는 Paper 보드 01(AT-SH-001)의
      규범 개정 지목과 사용자 지시(2026-08-24). **접히지 않으면 여전히 탈락이다.**
  - **새 창을 열고 싶어지면 모드로 풀 수 있는지 먼저 본다.**
  - **이행 중(정직 기록)**: 위는 목표 구조다. 코드가 어디까지 왔는지는
    [`plan/codex-실행계획.md`](plan/codex-실행계획.md)가 실측으로 적는다. 옛 구조
    (대화 창 + 캔버스 창)를 서술한 문서는 `ui/round-1R/two-windows.md`이며,
    이행이 끝날 때까지 **둘 다 살아 있다** — 어느 쪽을 인용하는지 밝혀라.
- **리퀴드 글래스 재료 규범은 타협 불가**: 모든 표면이 유리 / 유리는 무채색(틴트 금지) /
  ...
  - **예외 1건(2026-08-24 리프 1.3.2, 사용자 지시)**: 알림 오브 창의 **발화 상태에 한해**
    딥블루 바이저를 허용한다. 평소 오브는 무채색이고 미확인 알림이 있을 때만 색이
    드러난다 — **색이 장식이 아니라 신호다**([`ui/brand.md`](ui/brand.md)의 "무채색 구조가
    신호가 되는 순간"). 바이저는 유리 **위의 틴트가 아니라** 백색 셸에 뚫린 별개 표면이라
    유리 자체는 끝까지 무채색이다. 색값 3정지점은 `ui/kiwoome.zip` 원본 렌더 실측이고
    근거는 [`ui/kiwoome-reference.md`](ui/kiwoome-reference.md)에 있다.
    **이 예외는 오브 창에만, 발화 상태에만 적용된다** — 셸 창으로 새면 규범 위반이다.
...
- **용어는 [`GLOSSARY.md`](GLOSSARY.md)를 따른다.** 새 말을 만들면 코드에 쓰기 전에 용어집에 먼저
  ...
  "창", "캔버스", "detail".
- **2026-08-17 확정된 용어 4건.** 옛 문서에 반대 용법이 남아 있으니 인용할 때 주의한다.
  - **커맨드바 = 대화 창.** 대화 창 안의 부품이 아니라 같은 것의 다른 이름이다.
  - **캔버스 = 캔버스 창.** 창 안의 내용물은 **카드**다. "캔버스 12종" → **"카드 12종"**.
  - **카드는 데이터 표현 공통 UI다** — Power BI/Tableau처럼 **들어오는 데이터가 달라져도 UI는
    안 바뀐다.** TR이 늘었다고 카드를 늘리지 마라. 12종에 접는다.
  - **"모델 창"은 없다.** 모델 지정은 설정 안의 항목이다.

...
- **창은 둘뿐이다. 예외 없다.** 정의는 [`GLOSSARY.md`](GLOSSARY.md) §1.
  ...
  - **그 외의 모든 화면은 대화 창의 모드다** — 온보딩 · 인증 · 설정 · 주문 확인. 새 창을 만들지
    않는다. 설정 진입은 입력 영역의 **점(`#dot`)** 또는 커맨드바다.
  - **모드는 전부 대화 창 안의 형제 패널이다** — 온보딩·인증은 `#onboard`, 설정은 `#settings`.
    열리면 `#app`이 물러나고 창이 `chatMaxH`로 자란다. 셋 다 같은 리사이즈 문법이다.
    미착수인 모드는 **주문 확인** 하나다 (2026-08-16 실측).

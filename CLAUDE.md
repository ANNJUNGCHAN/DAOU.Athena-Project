...
- **창은 둘뿐이다. 예외 없다.** 정의는 [`GLOSSARY.md`](GLOSSARY.md) §1.
  - **대화 창**(= 채팅창 = **커맨드바**, 유일한 입력 지점) · **캔버스 창**(유일한 출력 지점).
    **캔버스 창 안에 뜨는 것은 "카드"다**(12종 고정). "캔버스"는 창을 뜻하지 내용물이 아니다.
    OS 레벨로 분리된 독립 창. 두 창 다 `frame: false` — **타이틀바가 없다**(`app/main.js`의 `commonWinOpts`).

...
- **표면은 상시 창 2 + 일시 표면 2다.** 정의는 [`GLOSSARY.md`](GLOSSARY.md) §1.
  - **상시 창** — **대화 창**(= 채팅창, 유일한 입력 지점) · **캔버스 창**(유일한 출력 지점).
    OS 레벨로 분리된 독립 창. 현재 코드에 존재하는 건 이 둘뿐이다.
  - **일시 표면** — **설정창**(설정·모델·주기·계좌. 커맨드바로 부르고, 대화 창의 **빨간 점멸**로도
    간다) · **팝업창**(**주식 주문을 진행할 때** 뜬다. 감시는 알림까지만이고 사람이 집행하는 지점).
    **둘 다 아직 미구현이다.**
  - 일시 표면은 창 개수에 세지 않는다. 즉시 탈락 조건: **상시 창** 3개 이상, 상시 사이드바/탭바/툴바,
    커맨드바 외 경로로**만** 접근 가능한 설정(빨간 점멸은 추가 진입로이지 유일 진입로가 아니다).

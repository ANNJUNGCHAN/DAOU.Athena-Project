...
> 06~08쪽(오브 시안 6종 · 유리 5단 변화 · 글자 5단 변화), 09~11쪽(에이전트 모드 ·
> 감시 빌더 · 알림 설정)도 같은 방식으로 파생시켰다.
>
...
16. **`--color-k-dim` 계열을 다크 Mockup Area 위에 직접 쓰지 마라** (2026-08-23,
    07·08쪽 판정 줄). 목업 안쪽(백색 유리 위)에서는 `--color-k-dim`(#5b6270)이 정상
    본문색이지만, 유리 밖 다크 지면(#0A0B0E + 데스크톱 그라디언트) 위에 같은 토큰을
    쓰면 대비가 무너져 **거의 안 보인다.** 유리 밖 캡션·판정 텍스트는 기존 보드 관행대로
    `#FFFFFF8A` / `#F2F4F8` / `#FFFFFF5C`를 쓴다. 같은 이유로 `--color-ok`(#5fce3f)도
    다크 지면에서는 한 단계 밝은 값(#7FE05F)으로 올린다.
18. **flex 자식에 `min-width:0`을 주지 않으면 형제가 밀려난다** (2026-08-23 실측, 10쪽
    감시 빌더). `flex-grow:1; flex-basis:0%`인 중앙 캔버스 안에 고정폭 입력 필드를 여러 개
    넣었더니 캔버스의 min-content가 커져 **옆의 400px 채팅 패널이 밀려 잘렸다**(채팅에
    `flex-shrink:0`이 걸려 있는데도). CSS 기본값 `min-width:auto` 때문이며 브라우저와
    동일한 동작이지만, Paper에서는 창 레이아웃이 통째로 어그러져 원인을 찾기 어렵다.
    → 3영역 셸처럼 형제 폭이 계약인 레이아웃에서는 **가변 영역에 `minWidth:0`을 먼저
    걸고**, 안쪽 필드는 고정폭 대신 `flexGrow`로 나눈다.
19. **`duplicate_nodes`의 `descendantIdMap`을 텍스트 정체성의 근거로 믿지 마라**
    (2026-08-23, 08쪽 배지). 맵이 반환한 자식 ID로 `set_text_content`를 걸었는데 화면에
    나온 번호가 좌우 반대였다. 맵 자체는 정확하지만 **어느 프레임이 어느 좌표에 있는지는
    별개 정보**이고, 여러 번 복제·재배치를 거치면 머릿속 대응이 어긋난다. 배지처럼 위치와
    내용이 짝을 이뤄야 하는 요소는 `get_node_info`로 `parentId`를 한 번 확인하고 쓰는 것이
    값싸다(실제로 이 방법으로 잡았다).

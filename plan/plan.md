...
> 최종 갱신: 2026-08-18 · 브랜치 `장중` — 전수 조사([`장중_해야할것.md`](장중_해야할것.md)) 후
> 착수 순서 ①(보안·결함)·②(문서 동기화) + 백엔드 결선 2건을 실행했다. 2026-08-17까지의
> 병합 이력(작업 브랜치 4종 → `main`)은 §1 참조.
>
...
**2026-08-18 · 착수 순서 ①·② + 결선 2건 실행 후 — 이 수치가 현행이다:**

```
backend 전체:  645 passed, 0 failed   (151초)   ← 브레인 코디네이터·fts·cp949 수정으로 +31
...
app:
  npm test                              → fail 0 (48건, node --test)
  npm run verify                        → 검증 1~9 전부 통과 (검증7 단언 15개 전부 true)
                                          확장 p95 33.4ms / max 50.3ms · 수축 p95 16.8ms
```

> **한글 경로 워크트리에서의 수치다.** 2026-08-17까지의 수치는 ASCII 경로 전제였고,
> 한글 경로에서만 드러나는 cp949 환경 의존 결함 2건을 이번에 닫아서 경로와 무관하게
> 초록이 됐다 (§5 부수 기록 참조).

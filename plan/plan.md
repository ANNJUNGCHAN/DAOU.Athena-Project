...
실측:   캐시 적중 시 "삼성전자 차트" **222~253ms**(resolve 재서명→render-plan 실행·
        변환·푸시→WS 240봉 실수신, 결정론 답변까지). 첫 질의는 ~27s(아래 블록) 유지.
구조:   AITS canvas_intent_cache의 정당화 논리 이식 — "규칙 기반 신규 판단이 아니라
        이미 내린 LLM 판정의 재사용". 판정(resolve 인자·카드 구성)만 저장, 데이터는
        리플레이마다 새로 조회(신선도 무결). L1 완전일치만(정규화: 공백·대소문자·
        말미 문장부호). TTL 30분·상한 200·메모리 전용.
        ① backend POST /api/v1/canvas/render-plan(오퍼레이션 계약 326→327, LLM 비노출,
           주문 헤더 무수용) — plan 실행+변환+사이드 채널 푸시 원스톱. 순수 변환은
           athena_api/canvas_transform.py로 이동(게이트웨이가 역수입 — 단일 소재지)
        ② 앱 main: 판정 캡처(스트림의 resolve·render tool_use 인자, 성공 왕복만 저장)
           + 진입점 리플레이(query-cache.js/fast-path.js — 실패는 무효화 후 정상 폴백)
        ③ 답변은 결정론 템플릿 + "이전 해석 재사용" 상시 고지(새 판단인 척 금지)
철회 1건: v3e "describe 생략" 지침 — W3 실측(생략 시 인자 추측→resolve 거부 루프,
        턴 18→31 악화)으로 이미 기각된 규칙임을 기존 고정 테스트가 잡아냄. 원복.
검증:   backend 852·ruff·generate current / app 238(query-cache 5·fast-path 4) /
        verify 전 단언. 배타 E2E: 리플레이 222ms + WS 카드(240봉) 수신 — 첫 검증에서
        수신 0은 실행 중이던 실제 앱이 단일 소비자 큐를 선점한 것(설계 확인).
```

**2026-08-19 (캔버스 우선 아키텍처) · 데이터 지름길 결선 후 재실측:**

```
실측:   "삼성전자 차트" E2E 166.5s(원 결함) → 108s(트리밍) → **27.3s** (6.1배).
        셀렉터 3툴(search/describe/resolve)만 사용 — athena_call 소멸, 데이터가
        모델을 한 번도 통과하지 않음. 사이드 채널로 240봉 실수신(probe WS 구독).
구조:   사용자 지시 "캔버스 우선 구성 → 필요 정보만 뽑아 답변"의 실결선.
        ① render_canvas가 plan_token을 받아 게이트웨이가 직접 실행·변환(canvas_data.py,
           결정적 매핑 ka10081 실측 기준·시간 오름차순·240봉 상한·주문 헤더 무전송)
        ② 봉투는 백엔드 사이드 채널로 — POST /api/v1/canvas/push + /api/v1/ws/canvas
           (ws_auth 배타 2모드 + ws_pump 공용, 오퍼레이션 계약 326). 툴 결과에는
           summary만(CLI 잘림 한도 실측 회피 — 1차 시도에서 봉투 인라인이 잘려 카드
           깨짐을 확인하고 전환) ③ 앱은 RoutineFeed 재사용으로 구독, 파서 'pushed'
           분류로 이중 렌더 방지 ④ 프롬프트 v3d(차트·표는 plan_token 직행)
부수 결함 2건 실측·수정: 토큰 설정 배포에서 앱 WS 피드(루틴 포함)가 인증 대기로
        조용히 죽는 문제 — backend-launcher.readLocalBearerToken()으로 앱이 backend/.env
        토큰을 메모리 조달(값 무로깅), 루틴 피드에도 동일 배선. path require 누락도 수정.
검증:   backend 852(canvas_data 8·canvas_push 6 포함) · ruff · generate current ·
        app 229(파서 pushed·토큰 리더) · verify 검증 1~19 전 단언 통과.
```

**2026-08-19 (창 이동 표준화) · 커서 폴링 드래그 → 네이티브 캡션 전환 후 재실측:**
...
변경: 본문 드래그 폐기(텍스트 선택 복원) → 손잡이는 **두 창 공통 맨 위 32px
     ...
     프롬프트 v3c(실사용 결함): "삼성전자 차트" 질의가 키움 4툴 성공 후 Bash 파이썬
     후처리로 새서 존재하지 않는 "권한 승인" 절차를 약속하며 멈춤(감사 로그로 확정 —
     키움 호출 자체는 12:46 UTC 전건 성공). 실행 환경 제약 절 신설(Bash·파일 접근
     없음·잘린 결과는 받은 만큼으로 카드) + 고정 테스트 1건(224→225). 재현 프로브
     probe_kiwoom_chart 통과(차트 카드 렌더·Bash 우회 0건·86s — 확률적 실패라 1회
     초록은 증명이 아니라 방향 증거, P0 프롬프트 규율과 같은 접근).
     근거: ui/round-1R/two-windows.md 정정 5(개정). backend 무접촉.
차트 지연(사용자 보고 "166초가 말이 되냐"): 원인 실측 — ka10081 일봉이 600행·
     134,080자(최신이 앞, first=20260819)를 반환해 CLI 툴 한도 초과 → 모델이
     일→주→월→연봉 4TR×13호출을 헛돎(13:05~13:08 감사, 백엔드 몫은 호출당
     61~125ms뿐). 수정: 게이트웨이 athena_call 트리밍(_trim_call_payload —
     직렬화 40k자 초과 시 최대 배열 앞쪽만 유지 + _athena_trimmed 정직 마커,
     REST 불변·LLM 표면만). 단위 3건(backend 835→838)·재프로브 108s(2사이클,
     4TR 헛돌기 소멸, 차트 20봉 렌더). 잔여 병목은 claude -p 모델 왕복 구조
     (사이클당 30~50s) — 다음 수 24와 동근.
```

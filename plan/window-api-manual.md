...
| **캔버스 창 ↔ 셀렉터 배선** | **미착수 (2026-08-18 재확인).** 이 문서가 설계한 FastAPI 4-tool 직접 호출은 여전히 0건이다(`app/` grep에 `athena_search` 등 부재). 2026-08-17 실배선(`claude -p` spawn → MCP 게이트웨이, plan.md 결정 D1)으로 캔버스에 실데이터가 닿게 됐지만 그건 **별개 아키텍처**이지 이 배선의 구현이 아니다 — 4-tool 직접 경로가 여전히 필요한지는 D1 이후 재판정 대상 |

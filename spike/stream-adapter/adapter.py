"""
과제 C — NAVER/DART 스트림 정규화 어댑터 (프로토타입)

`spike/captures/S2B-naver-news-*.json` (뉴스, `search_news`) 와
`spike/captures/S2B-jjlabsio-disclosure-list.json` (공시, `get_disclosure_list`)
캡처 데이터를 통일된 스트림 레코드로 정규화한다.

이 모듈은 나중에 `backend/athena_mcp/` 게이트웨이로 승격될 로직의 프로토타입이다.
새로 API를 호출하지 않는다 — 전량 캡처 파일 기반.
"""
...
# 넣은 리터럴 텍스트가 조용히 손실되는 취약점이다. 이 순서를 test_adapter.py가
# 고정한다.
...
if __name__ == "__main__":
    news_date = [normalize_news_item(it) for it in load_news_items(CAPTURES_DIR / "S2B-naver-news-date.json")]
    news_sim = [normalize_news_item(it) for it in load_news_items(CAPTURES_DIR / "S2B-naver-news-sim.json")]
    filings = [normalize_filing_item(it) for it in load_filing_items(CAPTURES_DIR / "S2B-jjlabsio-disclosure-list.json")]

    combined = sorted(news_date + filings, key=sort_key, reverse=True)
    print(f"combined records: {len(combined)}")
    print(f"first 3 (most recent):")
    for r in combined[:3]:
        print(f"  [{r['ts_precision']}] {r['ts']} {r['source']} :: {r['title'][:40]}")

    deduped_date, stats_date = dedupe(news_date)
    deduped_sim, stats_sim = dedupe(news_sim)
    print("date dedupe stats:", stats_date)
    print("sim dedupe stats:", stats_sim)
